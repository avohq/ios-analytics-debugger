// Generated by Avo VERSION 57.18.0, PLEASE EDIT WITH CARE
...
+ (void)invoke:(NSString *)eventId hash:(NSString *)hash messages:(NSArray *)messages {
  ...
  if (samplingRate > 0) {
    if (drand48() < samplingRate) {
      NSLog(@"invoke MIjoyq5YKfqzODtIukQz master 0cd8DLUxoxnhXaqRxL6O DQ9ku0SjZ");
      NSDictionary *payload = @{
        @"ac": @"MIjoyq5YKfqzODtIukQz",
        @"br": @"master",
        @"en": @"dev",
        @"ev": eventId,
        @"ha": hash,
...
+ (void)invokeMeta:(NSString *)type messages:(NSArray *)messages {
  if (samplingRate > 0) {
    if (drand48() < samplingRate) {
      NSLog(@"invokeMeta MIjoyq5YKfqzODtIukQz master 0cd8DLUxoxnhXaqRxL6O DQ9ku0SjZ");
      NSDictionary *payload = @{
        @"ac": @"MIjoyq5YKfqzODtIukQz",
        @"br": @"master",
        @"en": @"dev",
...
+ (NSArray *)assertUserEmail:(nonnull NSString *)userEmail
{
  NSMutableArray *messages = [[NSMutableArray alloc] init];
  [messages addObjectsFromArray:[self assertNonOptional:@"-rU9uRFvXe" propertyName:@"User Email" value:userEmail]];
  return messages;
}
...
+ (NSArray *)assertUserId_:(nonnull NSString *)userId_
{
  NSMutableArray *messages = [[NSMutableArray alloc] init];
  [messages addObjectsFromArray:[self assertNonOptional:@"avo-enriched-type-user-id" propertyName:@"User Id" value:userId_]];
  return messages;
}
 ...
 */
+ (void)loginWithUserId:(nonnull NSString *)userId_
  userEmail:(nonnull NSString *)userEmail
{
  // assert properties
  if (__ENV__ != AVOEnvProd || [self mobileDebuggerEnabled]) {
    NSMutableArray *messages = [[NSMutableArray alloc] init];
    [messages addObjectsFromArray:[self assertUserId_:userId_]];
    [messages addObjectsFromArray:[self assertUserEmail:userEmail]];
    // debug console in Avo
    [AvoInvoke invoke:@"LXOJCcMv3f" hash:@"85d91ef1ef8bd66a2ca594eaee903cfa93eeab2f3dff3865a6221d58f1f394b7" messages:messages];
    if ((__ENV__ != AVOEnvProd && __DEBUGGER__ != nil) || (__ENV__ == AVOEnvProd && [self mobileDebuggerEnabled])) {
      // Avo mobile debugger
      NSArray<NSDictionary *> * eventProps = @[];
      NSArray<NSDictionary *> * userProps = @[
        @{@"id" : @"avo-enriched-type-user-id",@"name" : @"User Id",  @"value" : userId_ ? [userId_ description] : @"nil"}, 
        @{@"id" : @"-rU9uRFvXe",@"name" : @"User Email",  @"value" : userEmail ? [userEmail description] : @"nil"}];
      [self mobileDebuggerPostEvent:@"Login" withTimestamp:[NSNumber numberWithDouble:[[NSDate date] timeIntervalSince1970]] withId:@"LXOJCcMv3f" withEventProps:eventProps withUserProps:userProps withMessages:messages];
    }
    if (__ENV__ != AVOEnvProd) {
      if (__STRICT__) {
        if ([messages count] > 0) {
  ...
  if (__ENV__ != AVOEnvProd) {
    ...
    NSMutableDictionary *avoLogUserProperties = [[NSMutableDictionary alloc] init];
    if (userEmail != nil) {
      avoLogUserProperties[@"User Email"] = userEmail;
    }
    
    [self avoLoggerLogEventSent:@"Login" eventProperties:avoLogEventProperties userProperties:avoLogUserProperties];
  }
  
  NSDictionary *customEventProperties = @{};
  
  NSMutableDictionary *customUserProperties = [[NSMutableDictionary alloc] init];
  if (userEmail != nil) {
    customUserProperties[@"User Email"] = userEmail;
  }
  
  [custom identify:@"userId_"];
  [custom setUserProperties:@"userId_" withUserProperties:customUserProperties];
  [custom logEvent:@"Login" withEventProperties:customEventProperties];
}
...
+ (void)logout
{
   ...
   if (__ENV__ != AVOEnvProd || [self mobileDebuggerEnabled]) {
     NSArray *messages = @[];
     // debug console in Avo
     [AvoInvoke invoke:@"rgk9vXIWk" hash:@"c2fd3175a8ab20c260b3a0c5e8d548306aedb7a488a46f77c042efeb4ee9ca2e" messages:messages];
     if ((__ENV__ != AVOEnvProd && __DEBUGGER__ != nil) || (__ENV__ == AVOEnvProd && [self mobileDebuggerEnabled])) {
       // Avo mobile debugger
       NSArray<NSDictionary *> * eventProps = @[];
       NSArray<NSDictionary *> * userProps = @[];
       [self mobileDebuggerPostEvent:@"Logout" withTimestamp:[NSNumber numberWithDouble:[[NSDate date] timeIntervalSince1970]] withId:@"rgk9vXIWk" withEventProps:eventProps withUserProps:userProps withMessages:messages];
     }
   }
   ...
   if (__ENV__ != AVOEnvProd) {
     ...
     [self avoLoggerLogEventSent:@"Logout" eventProperties:avoLogEventProperties userProperties:avoLogUserProperties];
   }
   ...
   [custom logEvent:@"Logout" withEventProperties:customEventProperties];
   [custom unidentify];
 }
